...
**Use**: Track foot traffic at retail, restaurants, theme parks
...
2. **Preprocessing**:
   - Cloud removal (mask or filter cloudy pixels)
   - Atmospheric correction (remove haze)
   - Georeferencing (align to coordinates)
   - Normalization (consistent brightness/contrast)
...
3. **Align with fundamentals**:
   - Retail traffic → same store sales
   - Tank levels → EIA inventory reports
   - NDVI → USDA yield estimates

4. **Measure correlation**:

   \\[\\text{IC} = \\text{Corr}(\\text{Satellite Feature}_t, \\text{Fundamental}_{t+k})\\]

5. **Test predictiveness**:
   - Does it predict before official data?
   - How much lead time (days/weeks)?
